...
public interface RequirementMapper extends BaseMapper<Requirement> {
    ...
    @Select({
            ...
            "      OR (",
            "        r.department_id = #{userDepartmentId}",
            "        AND r.creator_role_codes IS NOT NULL",
            "        <if test='roleCodes != null and roleCodes.size() &gt; 0'>",
            "          AND (",
            "            <foreach collection='roleCodes' item='rc' separator=' OR '>",
            "              JSON_CONTAINS(r.creator_role_codes, JSON_QUOTE(#{rc}))",
            "            </foreach>",
            "          )",
            "        </if>",
            "      )",
            ...
            "              JSON_CONTAINS(dmr.manager_role_codes, JSON_QUOTE(#{rc2}))",
            "            </foreach>",
            "          )",
            "      )",
            "    </if>",
