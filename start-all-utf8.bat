...
::   需求管理系统 - 唯一启动入口
::   用法:
::     start-all.bat                     一键启动全部服务（隐藏窗口后台运行，前后端日志输出到 logs/ 目录）
::     start-all.bat check               健康检查（不启动应用）
::     start-all.bat down                停止前后端进程 + 所有 Docker 容器
::     start-all.bat logs <backend|frontend>   实时查看前后端日志
...
:: JDK 17+ 的 jansi 需要放开模块访问权限，避免启动警告
...
echo   Docker 容器已就绪，跳过 compose up
...
echo   后端端口 %BACKEND_PORT% 已在监听，复用现有服务
goto after_backend
:start_backend
echo   后端启动中（隐藏窗口），日志: logs\backend.log
...
echo   前端端口 %FRONTEND_PORT% 已在监听，复用现有服务
goto after_frontend
:start_frontend
echo   前端启动中（隐藏窗口），日志: logs\frontend.log
...
echo     start-all.bat e2e        全链路 E2E
...
if "%FAILED%"=="0" (
    ...
    echo   ERR 有 %FAILED% 个服务连接失败，建议执行 start-all.bat 启动
    exit /b 1
)
...
if not exist "%TARGET%" (
    echo 日志文件不存在: %TARGET%
    echo 服务可能未启动，先执行 start-all.bat
    exit /b 1
)
...
echo   从 http://localhost:%BACKEND_PORT%/v3/api-docs 抓取...
...
if errorlevel 1 (
    echo   抓取失败，请先执行 start-all.bat 启动后端
    exit /b 1
)
echo   OK 已保存到 %ROOT_DIR%contracts\openapi\openapi.latest.json
exit /b 0
...
if %ITER% GEQ 30 (
    echo     TIMEOUT  %LABEL% 30 秒内未就绪，继续（不阻塞）
    goto :eof
)
...
if "%HEALTH%"=="healthy" (
    echo   OK %NAME% (端口 %PORT%) - 连接正常
) else if "%HEALTH%"=="no-healthcheck" (
    echo   OK %NAME% (端口 %PORT%) - 连接正常
) else (
    echo   WARN %NAME% (端口 %PORT%) - 健康检查未通过 (%HEALTH%)
)
goto :eof
