...
- **状态只允许通过工作流执行**：需求通用更新接口不得直接修改 `status`
- **按当前节点返回可执行动作**：需求详情页仅展示当前用户在当前节点下有权限执行的流转目标
- **流转前校验**：执行流转时同时校验节点权限、流转角色限制、必填字段、条件表达式
- **流转后联动**：自动记录状态变更历史，并向创建人/负责人发送通知
- **删除保护**：仅未发生过工作流流转记录的需求允许删除，避免绕过流程销毁已进入执行链路的数据
...
- **草稿编辑模式**：页面内编辑的都是草稿定义，不直接写运行态状态表/流转表
- **节点类型**：
...
- **发布生效机制**：
  - **保存版本**：生成 `workflow_versions` 快照，不影响运行中的需求
  - **启用版本**：按版本定义重建运行态 `workflow_states` 与 `workflow_transitions`
  - **启用保护**：启用前校验当前项目中已存在需求的状态是否仍被新版本覆盖
...
| 权限维度 | 配置项 | 说明 |
|----------|--------|------|
| **操作权限** | 可操作角色 | 哪些角色可以在此节点执行操作，可与“可操作人”并行配置，满足任一条件即可执行 |
...
| 节点权限 | 各状态下的可操作角色、可操作人、动态角色 |
| 必填字段 | 流转到特定状态时必须填写的字段 |
| 自动流转 | 满足条件时自动流转到下一状态 |
| 通知规则 | 状态变更时通知相关人 |
| 版本发布 | 保存版本与启用版本分离，启用后才同步到运行态 |
| 配置权限 | 仅超级管理员或具备 `workflow:config` 权限的账号可管理流程配置 |

#### 4.2.2a 工作流治理规则

- **单一事实来源**：需求状态定义与状态间流转仅来源于可视化工作流版本，不允许再由独立状态维护页面直接修改运行态
- **运行态只读心智**：`workflow_states`、`workflow_transitions` 是流程版本启用后的运行态投影，不作为人工长期维护入口
- **权限分层**：
  - **配置权限**：仅超级管理员或 `workflow:config` 角色可编辑/保存/启用工作流
  - **执行权限**：普通业务用户仅能在需求详情页执行当前节点授权范围内的流转
- **节点权限表达方式**：每个节点至少支持按角色授权或按具体人授权，允许两者并存
- **兼容保护**：启用新版本时，如现有需求状态无法映射到新版本节点，则禁止发布并提示缺失状态
...
| GET | `/api/v1/requirements/:id/available-transitions` | 获取当前用户对该需求可执行的流转列表 |
...
- `PUT /api/v1/requirements/:id` 不允许直接修改 `status`
- 工作流配置相关接口仅允许超级管理员或具备 `workflow:config` 权限的账号访问
