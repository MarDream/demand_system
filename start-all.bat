...
powershell -NoProfile -Command "$p=Start-Process cmd -ArgumentList @('/c','chcp 65001>NUL && set JAVA_TOOL_OPTIONS=-Dfile.encoding=UTF-8 -Dstdout.encoding=UTF-8 -Dstderr.encoding=UTF-8 && set MAVEN_OPTS=--enable-native-access=ALL-UNNAMED && set DEMAND_LOG_PATH=%LOG_DIR%&& cd /d %ROOT_DIR%demand_backend && %MVN_CMD% spring-boot:run -Dspring-boot.run.profiles=dev -Dspring-boot.run.arguments=--server.port=%BACKEND_PORT% -Dspring-boot.run.jvmArguments=\"--add-opens=jdk.unsupported/sun.misc=ALL-UNNAMED --sun-misc-unsafe-memory-access=allow\"') -WindowStyle Hidden -RedirectStandardOutput '%BACKEND_LOG%' -RedirectStandardError '%BACKEND_ERR%' -PassThru; Set-Content -Path '%BACKEND_PID%' -Value $p.Id -Encoding ascii"
:after_backend
