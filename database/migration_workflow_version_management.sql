...
ADD COLUMN `requirement_type_id` INT UNSIGNED DEFAULT NULL COMMENT '工单类型ID' AFTER `project_id`,
ADD INDEX `idx_requirement_type` (`requirement_type_id`);

-- 3. 增强 workflow_versions 表
ALTER TABLE `workflow_versions`
ADD COLUMN `deprecated_at` DATETIME DEFAULT NULL COMMENT '废弃时间',
ADD COLUMN `change_log` TEXT COMMENT '版本变更说明',
ADD COLUMN `submitted_for_approval_at` DATETIME DEFAULT NULL COMMENT '提交审批时间',
ADD COLUMN `approved_at` DATETIME DEFAULT NULL COMMENT '审批通过时间',
ADD COLUMN `approved_by` BIGINT UNSIGNED DEFAULT NULL COMMENT '审批人ID',
ADD COLUMN `approval_comment` TEXT COMMENT '审批意见',
ADD COLUMN `config_hash` VARCHAR(64) DEFAULT NULL COMMENT '配置哈希（排除名称/描述，用于判定实质性变更）',
ADD INDEX `idx_activation_status` (`activation_status`),
ADD INDEX `idx_project_active` (`project_id`, `is_active`, `activation_status`);
...
-- 6. 为现有项目绑定默认工单类型和工作流
-- 查询每个项目的最新活跃工作流版本，创建对应的项目级工单类型并绑定
...
-- 7. 为现有需求设置默认工单类型
-- 如果需求已绑定项目，使用项目级工单类型；否则使用系统级默认类型
...
-- 8. 为现有工作流版本补全 config_hash（数据迁移）
-- 注意：首次执行时，config_hash 为 NULL，后续发布时会自动计算
UPDATE `workflow_versions`
SET `config_hash` = `runtime_hash`
WHERE `config_hash` IS NULL AND `runtime_hash` IS NOT NULL;

-- 9. 更新 activation_status 枚举值说明（仅文档，无需执行）
-- draft: 草稿
