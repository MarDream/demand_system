...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''Column requirement_type_id already exists'' AS info');
...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''Column deprecated_at already exists'' AS info');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''Column change_log already exists'' AS info');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''Column submitted_for_approval_at already exists'' AS info');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''Column approval_comment already exists'' AS info');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 9. 添加索引 idx_activation_status（检查后添加）
SET @idx_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
                   WHERE TABLE_SCHEMA='demand_system' AND TABLE_NAME='workflow_versions'
                   AND INDEX_NAME='idx_activation_status');
SET @sql = IF(@idx_exists = 0,
    'CREATE INDEX `idx_activation_status` ON `workflow_versions` (`activation_status`)',
    'SELECT ''Index idx_activation_status already exists'' AS info');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 10. 添加索引 idx_project_active（检查后添加）
SET @idx_exists = (SELECT COUNT(*) FROM INFORMATION_SCHEMA.STATISTICS
                   WHERE TABLE_SCHEMA='demand_system' AND TABLE_NAME='workflow_versions'
                   AND INDEX_NAME='idx_project_active');
SET @sql = IF(@idx_exists = 0,
    'CREATE INDEX `idx_project_active` ON `workflow_versions` (`project_id`, `is_active`, `activation_status`)',
    'SELECT ''Index idx_project_active already exists'' AS info');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;
