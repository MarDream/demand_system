...
SET @sql = IF(@col_exists = 0,
    ...
    'SELECT ''Column requirement_type_id already exists'' AS ''Status''');
PREPARE stmt FROM @sql;
EXECUTE stmt;
DEALLOCATE PREPARE stmt;

-- 3. 增强 workflow_versions 表（仅添加缺失字段）
ALTER TABLE `workflow_versions`
ADD COLUMN IF NOT EXISTS `deprecated_at` DATETIME DEFAULT NULL COMMENT '废弃时间',
ADD COLUMN IF NOT EXISTS `change_log` TEXT COMMENT '版本变更说明',
ADD COLUMN IF NOT EXISTS `submitted_for_approval_at` DATETIME DEFAULT NULL COMMENT '提交审批时间',
ADD COLUMN IF NOT EXISTS `approved_at` DATETIME DEFAULT NULL COMMENT '审批通过时间',
ADD COLUMN IF NOT EXISTS `approved_by` BIGINT UNSIGNED DEFAULT NULL COMMENT '审批人ID',
